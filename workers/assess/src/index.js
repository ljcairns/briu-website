...
Six Layers Deep (/build/six-layers-deep/): Zero-trust security architecture for AI agents. Six independent layers: (1) Network isolation — gateway on loopback only, private VPN, invisible to internet. (2) Permission isolation — 4 agents with different permission levels enforced at infrastructure, not prompts. Main (Sonnet 4.6) full access, Heartbeat (Haiku 4.5) read-only, Sandbox (Haiku 4.5) classify-only. (3) Command allowlist — deny-by-default command allowlist, dangerous shell patterns blocked at hook level. (4) Sandbox architecture — untrusted content processed by isolated agent, output re-sanitized by jq script before main sees it, 16-pattern injection detection. (5) CVE monitoring every 2 hours + kill switch auto-locks on unpatched critical vuln. (6) Security context injected fresh every agent turn. For injection to succeed: must fool sanitizer + sandbox agent + jq extraction + main agent + the command allowlist. Each layer independent.
