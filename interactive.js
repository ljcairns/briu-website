...
(function() {
  ...
  function fetchChat(callback) {
    ...
    .then(function(data) {
      ...
      if (data.sessionId) { sessionId = data.sessionId; try { localStorage.setItem(SESSION_KEY, sessionId); } catch(e) {} }
      callback(data);
    })
    .catch(function(err) {
            ...
            try {
              ...
              if (evt.type === 'delta') {
                if (!streamEl) {
                  ...
                  streamEl.className = 'conv-msg conv-msg-assistant';
                  streamEl.innerHTML = '<p></p>';
                  if (thread) thread.appendChild(streamEl);
                }
                text += evt.text;
                var p = streamEl.querySelector('p');
                if (p) p.innerHTML = formatMessage(text).replace(/^<p>/, '').replace(/<\/p>$/, '');
                scrollThread();
              }
              if (evt.type === 'done') {
                clearTimeout(timer); isWaiting = false;
                if (streamEl) streamEl.remove();
                if (evt.sessionId) { sessionId = evt.sessionId; try { localStorage.setItem(SESSION_KEY, sessionId); } catch(e) {} }
                callback(evt);
                return;
              }
              if (evt.type === 'error') {
